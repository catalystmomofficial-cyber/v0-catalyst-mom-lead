...
export function PostpartumResultsClient({ score, tier, quizState }: { score: number; tier: string; quizState: any }) {
  // --- 1. Sanitize User Name Input ---
  const rawName = quizState.userName || quizState.name || ''
  const cleanName =
    rawName.trim().length < 2 || /^[bcdfghjklmnpqrstvwxyz]+$/i.test(rawName.trim()) || rawName.toLowerCase() === 'none'
      ? 'Mama'
      : rawName.trim()
